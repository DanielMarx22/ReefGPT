...
*   **The Goal**: Transition away from headless browser scraping (Playwright) to a scalable, TOS-compliant data retrieval method before official public launch, while maintaining global access for the user.
*   **The Plan**: Currently, ReefGPT acts as a cloud headless browser scraping `apexfusion.com`. At massive scale, this will cause heavy server load on Neptune and likely trigger Cloudflare bot protections. To prevent IP bans and ensure users can check their tanks from anywhere in the world, we must explore these transition strategies:
    1.  **Official Partner API (Primary Goal)**: Reach out to Neptune Systems directly to secure an official partner API key. This is how established third-party companies pull cloud data safely and legally without scraping. This ensures 100% global uptime for users away from home.
    2.  **Reverse-Engineering Private Cloud APIs**: As an intermediate step, many smart-home integrations (like Home Assistant) reverse-engineer Neptune's private REST/WebSocket APIs rather than rendering a full headless browser. This is much lighter on servers but still carries TOS risk.
    3.  **Local Network Relay (Fallback)**: Polling the Apex unit's local XML/JSON endpoints (`http://apex.local/cgi-bin/status.xml`). While this is perfectly safe and bypasses the cloud entirely, it requires the user to run a lightweight relay program 24/7 on a computer inside their house to push the local data up to our Supabase cloud. This is a high-friction setup for casual users.
